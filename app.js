document.addEventListener('DOMContentLoaded', () => {
  ...
  const apps = [
    ...
    { id: 'resume', title: 'Resume', iconClass: 'icon-folder', content: generateResumeContent() },
  ...
  function generateResumeContent() {
    return `
      ...
      <div style="flex: 1; width: 100%; background: white; display: flex; flex-direction: column;">
        ...
        <div class="mobile-pdf-fallback" style="display: none; flex-direction: column; flex: 1; width: 100%; background: white;">
          <iframe id="mobile-docs-viewer" style="flex: 1; width: 100%; border: none;" scrolling="yes"></iframe>
          <div style="padding: 15px; border-top: 1px solid var(--win-border); background: #f8f9ff; display: flex; justify-content: center;">
            <a href="#" onclick="window.forceDownloadResume(event, 'Resume%202.0.pdf', 'Akshay_Chandel_Resume.pdf')" class="win-btn primary-btn" style="text-decoration: none; font-size: 15px; padding: 10px 24px; border-radius: 99px;">Download PDF</a>
          </div>
        </div>
      </div>
      ...
      <script>
        // Set the Google Docs Viewer URL dynamically based on the current domain
        setTimeout(() => {
          const mobileViewer = document.getElementById('mobile-docs-viewer');
          if (mobileViewer) {
            // Google Docs Viewer requires a public URL. If on localhost, it won't load the PDF.
            const publicUrl = window.location.hostname === 'localhost' || window.location.protocol === 'file:' 
              ? 'https://akshay-os-chi.vercel.app/Resume%202.0.pdf' 
              : window.location.origin + '/Resume%202.0.pdf';
            
            mobileViewer.src = 'https://docs.google.com/viewer?url=' + encodeURIComponent(publicUrl) + '&embedded=true';
          }
        }, 50);
      </script>
    `;
